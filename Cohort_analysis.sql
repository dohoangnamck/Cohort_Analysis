WITH 
t0 AS (		-- cohort theo tháng nên lọc data 1 năm
    SELECT *
    FROM SuperStoreSales_Whole  
    WHERE YEAR(Order_Date) = 2018
)
--SELECT * FROM t0
, t1 AS (	-- lọc ra các cột cần thiết và tạo cột tháng mà kh mua hàng (Order_Month) theo format 'yyyy-MM-01'-đây ko phải dạng Date
    SELECT Order_ID, Order_Date, Customer_ID, Sales,  FORMAT( Order_Date , 'yyyy-MM-01' ) Order_Month
    FROM t0
    )
--SELECT * FROM t1
, t2 AS (	-- GROUP BY Customer_ID và tính ngày mua hàng đầu tiên của mỗi kh (Cohort_Month)
    SELECT Customer_ID, FORMAT(MIN(Order_Date), 'yyyy-MM-01' ) Cohort_Month
    FROM t0
    GROUP BY Customer_ID
)
--SELECT * FROM t2
, t3 AS (	-- join t1 và t2, tính CohortIndex theo month bằng DATEDIFF sau khi format lại ở dạng date với CAST
...
--SELECT * FROM t3
, t4 AS (   -- lọc ra các cột cần thiết và COUNT Customer_ID. GROUP BY bnh cột thì Select bấy nhiêu
    SELECT Cohort_Month, Order_Month, CohortIndex, COUNT(DISTINCT Customer_ID) AS Count_CustomerID
    FROM t3
    GROUP BY Cohort_Month, Order_Month, CohortIndex 
)
--SELECT * FROM t4
, t4_2 AS (	  -- lọc ra các cột cần thiết và sum(sales)
    SELECT Cohort_Month, Order_Month, CohortIndex, SUM(sales) Total_Sales
    FROM t3
    GROUP BY Cohort_Month, Order_Month, CohortIndex     
)
--SELECT * FROM t4_2
, t5 AS (  --lập bảng PIVOT
    SELECT *
    FROM 
    (
    ...
    PIVOT(   --pivot theo sum(Count_CustomerID)
